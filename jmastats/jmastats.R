...
load(here::here("jmastats", "stations.rda"))
...
pack_df <- function(df, unpack = FALSE) {
  if (unpack == FALSE) {
    df <-
      df |>
      tidyr::pack(
        atmosphere = tidyselect::starts_with("atmosphere"),
    ...
    df[, c(
      names(purrr::discard(df, tibble::is_tibble)),
    ...
    df |>
      tidyr::unpack(tidyselect::everything(), names_sep = "_")
  }
}
...
jma_collect_raw <- function(item = NULL, block_no, year, month, day, quiet) {
  ...
  if (item == "annually") {
    ...
    df <-
      df |>
      tidyr::pivot_longer(cols = -c(1, ncol(df)), names_to = "rank") |>
      tidyr::extract(
        col = value,
        into = c("value", "date"),
        regex = "(.+)\\((.+)\\)"
    ...
    item %in% c("nml_ym", "nml_3m", "nml_10d", "nml_mb5d", "nml_daily")
  ) {
    element <- NULL
    ...
    cat(
      cli::col_br_blue(
        paste(
          ...
          "to",
          nml_meta$years[2],
          paste0("(", nml_meta$records, " years of data).\n")
        )
      )
    )
    df <-
    ...
    if (item %in% c("nml_10d", "nml_mb5d")) {
      ...
      df <-
        df |>
        dplyr::mutate(element = paste0(month, intToUtf8(26376), element))
    }
  }
  tibble::as_tibble(df)
}
...
slow_jma_collect <-
  purrr::slowly(
    function(item, block_no, year, month, day, quiet) {
      jma_collect_raw(
        item = item,
        block_no = block_no,
        year = year,
    ...
    rate = purrr::rate_delay(pause = 1.5),
    quiet = FALSE
  )
  ...
  df |>
    ...
    dplyr::mutate(
      date = as.Date(paste(
        ...
        stringr::str_pad(date, width = 2, pad = "0"),
        sep = "-"
      ))
    )
}
...
jma_url <- function(item = NULL, block_no, year, month, day, ...) {
  ...
  selected_item <-
    rlang::arg_match(
      ...
      c(
        ...
        "10min",
        "rank",
        paste("nml", c("ym", "3m", "10d", "mb5d", "daily"), sep = "_")
      )
    )
  ...
  if (
    selected_item %in% c(paste("nml", c("ym", "3m", "10d", "mb5d"), sep = "_"))
  ) {
    list(
      url = as.character(stringr::str_glue(
        ...
        selected_item = stringr::str_replace(
          selected_item,
          "_",
          dplyr::if_else(station_info$station_type == "a", "_amd_", "_sfc_")
        ),
      ...
      if (selected_item == "nml_daily") {
        list(
          url = as.character(stringr::str_glue(
            ...
            blockid = rlang::eval_tidy(.blockid),
            selected_item = dplyr::if_else(
              station_info$station_type == "a",
              "nml_amd_d",
              "nml_sfc_d"
            ),
            prec_no = station_info$prec_no
...
detect_station_info <- function(.blockid) {
  ...
  station_type <-
    ifelse(
      df_target_station$station_name == intToUtf8(c(39640, 26494)) &
        df_target_station$station_no == 23281,
      "a",
      station_type
    )
  station_type <-
    ifelse(
      df_target_station$station_name %in%
        c(
          intToUtf8(c(22899, 28288, 21029)),
...
note_vars <- function(var) {
  syms <-
    ...
    stringr::str_which(paste0(
      "(",
      paste0(
        c(intToUtf8(c(47, 47, 47)), intToUtf8(c(215)), intToUtf8(c(35))),
        collapse = "|"
      ),
      "|",
      "\\+|\\-|\\)|\\]",
...
jma_vars <- list(
  atmosphere = paste0("atmosphere_", c("land", "surface"), "(hPa)"),
  precipitation = paste0(
    "precipitation_",
    c("sum", "max_per_day", "max_1hour", "max_10minutes"),
    "(mm)"
  ),
  temperature = paste0(
    "temperature_",
    c("average", "average_max", "average_min", "max", "min"),
    "(\u2103)"
  ),
  humid = paste0("humidity_", c("average", "min"), "(%)"),
  wind = paste0(
    "wind_",
    c(
      paste0(c("average_speed", "max_speed"), "(m/s)"),
      "max_speed_direction",
  ...
  solar = paste0("solar_irradiance_", c("average"), "(MJ/m^2)"),
  snow = paste0("snow_", c("fall", "max_fall_day", "depth"), "(cm)"),
  cloud = paste0("cloud_covering_", c("mean")),
  condition = paste0("condition_", c("snow_days", "fog_days", "thunder_days"))
)
...
name_sets <- function(item) {
  switch(
    ...
    "daily_a1" = c(
      "date",
      paste0("precipitation_", c("sum", "max_1hour", "max_10minutes"), "(mm)"),
      paste0("temperature_", c("average", "max", "min"), "(\u2103)"),
      jma_vars$humid,
      paste0(
        "wind_",
        c(
      ...
      paste0("snow_", c("fall", "depth"), "(cm)")
    ),
    "daily_s1" = c(
      "date",
      paste0("pressure_", c("average", "sea_level"), "(hPa)"),
      paste0("precipitation_", c("sum", "max_1hour", "max_10minutes"), "(mm)"),
      paste0("temperature_", c("average", "max", "min"), "(\u2103)"),
      jma_vars$humid,
      paste0(
      ...
      paste0("sunshine_duration_", "(h)"),
      paste0("snow_", c("fall", "depth"), "(cm)"),
      paste0("weather", c("daytime (06:00-18:00)", "nighttime (18:00-06:00)"))
    ),
    "hourly_a1" = c(
      "time",
    ...
    "nml_3m_s" = c(
      ...
      paste0(
        "temperature_",
        c(rep("min_num_days_", 2), rep("max_num_days_", 4)),
        c("lt_0.0", "geq_35.0", "lt_0.0", "geq_25.0", "geq_30.0", "geq_35.0"),
        "(\u2103)"
      ),
      jma_vars$daylight,
      jma_vars$snow[c(1, 3)]
...
validate_date <- function(year, month, day) {
  check_positive <-
    sapply(list(year = year, month = month, day = day), function(x) {
      x > 0
    })
  ...
  if (is.na(x)) {
    rlang::inform(paste0(
      ...
      paste(
        names(which(
          c(
            "month" = ifelse(is.na(match(13, 1:12)), FALSE, TRUE),
            "day" = ifelse(is.na(match(1, 1:31)), FALSE, TRUE)
          ) ==
            FALSE
        )),
        collapse = ", "
...
check_input_coords <- function(longitude, latitude, geometry = NULL) {
  if (!is.null(geometry)) {
    if (!rlang::is_missing(longitude) || !rlang::is_missing(latitude)) {
      rlang::inform(
        "the condition assigned coord and geometry, only the geometry will be used"
      ) # nolint
    }
    ...
    list(
      absolute = list(
        colors = purrr::pmap_chr(
          list(
            r = c(180, 255, 255, 250, 0, 33, 160, 242),
...
split_period_char <- function(x) {
  stringr::str_split(
    x,
    paste0("(?=([0-9]{4}/[0-9]{1,2}|[0-9]{4}.+", intToUtf8(24180), "))"),
    simplify = TRUE
  ) |>
...
daylight <- function(date, lat,
  ...
  phi <- lat * pi / 180
  dip_deg <- 0.0293 * sqrt(pmax(alt_m, 0))
  h0 <- (h0_deg - dip_deg) * pi / 180
  
  cos_omega0 <- (sin(h0) - sin(phi) * sin(delta)) / (cos(phi) * cos(delta))
  cos_omega0_clipped <- pmin(pmax(cos_omega0, -1), 1)
  omega0 <- acos(cos_omega0_clipped)
  
  N <- (2 * omega0 * (180 / pi)) / 15
  return(N)
}
